...
原协议写死「每个案例派独立 subagent」，这是对执行环境的硬依赖——不少 Agent 运行时并不支持
子上下文。隔离的**目的**是「答案不进入执行上下文」，subagent 只是实现手段之一。按能力选档：

**A 档 — 支持 subagent（最优）**
每个案例派独立执行上下文，其中从未出现答案；持有答案的主上下文只在 Step 4 做比对。

**B 档 — 不支持 subagent，但可多轮/多会话**
双会话 + 文件闸门，效果与 A 档等价：
1. 会话一只获得 `meta.json` 与 ticker / 回放日 / 截断日，跑完 Step 1-3、单独提交 `verdict.json`；
2. **结束会话**（上下文清空）；
3. 由 `prepare_case.py --reveal <case>` 把答案文件解码落地到案例目录——**Step 3 提交之前，
   答案在工作区中物理上不存在**；
4. 会话二读答案做 Step 4。
上下文清空 + 文件不存在，双重保险。
...
只输出「命中 / 未命中 + 代号 diff」，**不把答案原文交给 AI**。根因分析基于机器输出而非
答案叙事。本档判别力略降（拿不到 `outcome_note` 的定性信息），但纪律最硬。

**三档共同的兜底（唯一可机器验证的时序证据）**
`verdict.json` 必须**单独 commit**，且该 commit 必须早于答案文件落地的 commit。
文件 mtime 会被 git 检出覆盖、自陈的 `frozen_before_diff` 字段可以事后补写，
只有 git 提交顺序无法伪造。**任何档位下，缺少这条提交顺序证据的案例，其结论不得写入
REPORT.md 作为战绩依据。**

**成对案例的额外要求**：神华/鞍钢（案例 7/8）、Netflix/Valeant（案例 9/21）、
中石油/福特（案例 20/23）这类成对样本必须**分别独立**执行、互不知晓对方存在——
否则第二个必然被第一个锚定，成对比对的判别力检验失效。B / C 档下这意味着成对案例
不能在同一会话内连续跑。
