...
此样例是 **v1 框架** 的真实产出，投委会章节用的是"三人评分卡 + 综合分 76"结构。v2（current）已把投委会重构为**三位大师独立评估**（各自从原始证据独立推导、不合成单一分数、显式呈现分歧，见 `references/investor-personas.md`），并新增了图表 vchart 溯源校验、capex 维持/扩张拆分、周期低位向上正常化。复核此样例时，报告的"三人评分雷达"与"综合分 76"应视为历史产物——新框架下同一份报告会呈现为三份独立评估全文 + 分歧对照表。

这正是 skill 演进的实证：v1 样例暴露了"评分换视角=同一人穿三件马甲"的问题，才有了 v2 的独立评估协议。其价值已沉淀进决策日志与 references 框架文件。

## 数据来源等级

年报/季报财务：SEC EDGAR XBRL（A 级）；行情与一致预期：金融数据接口 + 财报后媒体综述（A/B 级）；
