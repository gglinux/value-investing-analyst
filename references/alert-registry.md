...
同一算术事实可能因公司语境应作不同处理（告警 → 留痕降级）。语境缺失分三层，
每层对应不同的修复位置——**层次放错，修法就错**：

| 层 | 缺失内容 | 实例 | 修复位置 |
|---|---|---|---|
| 存在性 | 告警预设的对象不存在 | 分红幻觉（预设分红存在）打在零分红公司 | **前置**：写进告警判定逻辑（`cum_dividends ≤ 0` 即跳过并留 warning），不进豁免表 |
| 机制/方向 | 告警的因果叙事对该机制不成立 | OCF/FCF 低被读作烧钱恶化，实为主动再投入 | **豁免**：注册表条目（算术判据） |
| 事件性质 | 同一代码涵盖性质不同的算术形态 | 股本增加既是现金定增也是转增 | **配对判据**：见第六节三态判据 |

### 豁免注册表（事实源：`scripts/alert_codes.py` 的 `EXEMPTIONS`，本表为镜像）

| id | covers | 判据（同时满足，缺一不豁免） | 移除条件 |
|---|---|---|---|
| `EX_REINVEST_GROWTH` | M_DIVIDEND_ILLUSION / M_FCF_QUALITY / M_OWNER_YIELD_NOT_CASH_BACKED | ① OCF 序列无任何一年 ≤0（序列全空不豁免）；② 累计 capex ≥ 80% 累计 OCF；③ rows ≥4 年、最新年收入同比为正且 3 年收入 CAGR ≥10% | 第二例非 Netflix 类案例验证后收窄/泛化；周期股扩张期误豁免的代理误差一并复核 |

豁免动作：covers 内告警不进 `alert_codes`（机器轨不触发），降级 warnings 留痕；
`metrics_*.json` 的 `exemptions` 块登记命中证据（`fired`，判据字段值）与被抑制
清单（`suppressed`，{code, by}）——**抑制必须可审计，禁止静默吞掉**。

### 新增豁免的合法性五条（不满足的属叙事豁免，禁止入表）

1. **算术可判**：同底稿两人跑出同一豁免判定；
2. **证据独立**：判据字段与被豁免告警的触发字段不同源（否则是循环重述）；
3. **判据收窄**：只写语境的必要条件，禁止类型叙事（"成长股所以豁免"不成立）；
4. **抑制留痕**：降级 + exemptions 块登记，禁止静默吞掉；
5. **附移除条件**：判据何时收窄/泛化挂案例证据，防止豁免面只扩不缩。

### 与类型卡的分工（阶段 3-4，待估值语言轴案例锚）

类型卡只**提名**哪些告警值得语境审视（候选清单），裁决权始终在算术判据——
豁免挂类型标签而不挂算术，等于给「解释权自我扩张」开后门。断言轨的类型限定
语法（`must_not_trigger` 支持 `[type:...]`）同样后置。

**新增一条豁免时的流程**：① 在 `alert_codes.EXEMPTIONS` 声明四要素（先做，
否则引擎求值器挂不上）；② 在 `compute_metrics._EXEMPTION_EVALUATORS` 登记判据
求值器（缺失当场 KeyError）；③ 跑 `python3 tests/run_tests.py`（第 11 节守护
注册表自检与求值器同步）；④ 跑 `--rerun --baseline` 确认零漂移。
