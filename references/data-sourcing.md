...
0. **搬运完整性（最隐蔽的事故）**：抽取产物拿到了、底稿却没搬进去——比"数据源没有"更危险，因为分析师以为自己有。事故原型：GOOG/TSM 的 raw 产物 9 个年度全抓到，底稿只有 1 年有值，勾稽覆盖率 18% 而入口校验照打"通过"（raw 回填后 91%）。纪律：凡有抽取中间产物必须跑 `check_transcription.py` 比对；建稿脚本化优先。附带：`cash` 取 `cash_sti`（现金+短期投资）是估值口径，不要为过校验改狭义。
...
- **中概美股（PDD 实证）**：Futu 通道字段完整；**币种双口径**（`DisclosureCurrency=CNY`/`ShowCurrency=USD`）须按汇率与 20-F 人民币值反核；**现金口径**接口宽于披露，统一到披露口径；**利息收入剔除**（占税前 19%，从经营 OE 剔除）；**爬坡期正常化失真**（早年亏损拉低全期均值，双轨披露）。
...
- **港币口径陷阱**：港股接口利润表/资产负债表科目为港币（期末汇率折算），与披露人民币值差约 10%——按「接口值 ÷ 当年末汇率」反推，年报公告交叉验证误差 <1%。
...
- **周期高位警报在消费股同样适用**：泡泡玛特 2025 净利率 34.4% 为全期均值 1.60 倍，引擎自动判周期高位并强制正常化（92 亿而非峰值 128 亿）——消费股爆款周期标准形态。
- **"经营+投资"双轮公司**：腾讯投资组合 8751 亿 CNY——DCF 只评估经营业务，组合按折价（上市 9 折/非上市 6 折，或更保守 7/4 折）单独加回（`reverse_dcf.py --add-back/--deduct`）。
- **投资减值年扭曲均值**：腾讯 2023 归母 1152 亿 vs Non-IFRS 1577 亿（公允下修+联营减值）——spike_notes 须标注，双口径差异披露。
...
**转引否决优先于被转引对象**（REQ-P0-09）：含「转引/转载/…报道」且无可核验官方凭证时按 tier 5——「年报」在任何转引描述里都必然出现，不得据此升为官方源。例外（不降级）：①含监管 URL/公告编号；②明说该值已「弃用/不符/未采纳」；③官方为主源、接口终端仅作第二独立源。报表体裁词（审计报表/合并利润表等）孤证不升 tier 1，须伴随凭证（URL、公告号、披露日、页码）。

> 显式 `source_tier` **短路**推断，写错即固化。批量补录须复核辖区；守卫测试要传 `source` 文本走真实推断再比对——传条目 dict 会得到恒真断言（REQ-P0-09 根因）。
